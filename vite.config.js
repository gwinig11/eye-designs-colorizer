...
import logAirtableHandler from './api/log-airtable.js'
...
const localApiPlugin = () => ({
  ...
  configureServer(server) {
    ...
    server.middlewares.use('/api/log-airtable', async (req, res) => {
      try {
        req.body = await readJsonBody(req)
        await logAirtableHandler(req, createVercelLikeResponse(res))
      } catch (error) {
        res.statusCode = error instanceof SyntaxError ? 400 : 500
        res.setHeader('Content-Type', 'application/json')
        res.end(JSON.stringify({ error: error instanceof SyntaxError ? 'Invalid JSON body.' : 'Local API handler failed.' }))
...
export default defineConfig(({ mode }) => {
  ...
  const airtableApiKey = normalizeEnvValue(process.env.AIRTABLE_API_KEY) || normalizeEnvValue(env.AIRTABLE_API_KEY) || normalizeEnvValue(env.AIRTABLE_PAT) || normalizeEnvValue(env.AIRTABLE_TOKEN)
  ...
  if (airtableApiKey) {
    process.env.AIRTABLE_API_KEY = airtableApiKey
  }
